(() => {
  ...
  .qcap-table-wrap{overflow:auto}.qcap-table{border-collapse:collapse;width:100%;min-width:900px;font-size:12px}.qcap-table th,.qcap-table td{border:1px solid #24324a;padding:7px 8px;text-align:left;white-space:nowrap}.qcap-table th{position:sticky;top:0;background:#102033;color:#67e8f9;z-index:1}.qcap-pos{color:#4ade80;font-weight:800}.qcap-neg{color:#fb7185;font-weight:800}.qcap-warn{color:#facc15;font-weight:900}.qcap-muted{color:#94a3b8}.qcap-error{padding:14px;border:1px solid #7f1d1d;background:#450a0a;color:#fecaca;border-radius:8px}
  ...
  async function load(){
    ...
    try{
      ...
      const o=cap.overview||{},a=o.account||{},positions=o.positions||[],trades=o.closedTrades||[],stats=o.tradeStats||[];
      ...
      const cards=`<div class="qcap-card"><div class="qcap-title">DEMOKONTO ${esc(a.accountName||"")}</div><div class="qcap-cards">
        ...
        <div class="qcap-stat"><small>History</small><strong>7 Tage</strong></div>
      ...
      const simHtml=`<div class="qcap-card"><div class="qcap-title">SIZE-SIMULATION · NICHT AKTIV</div><div class="qcap-muted" style="margin-bottom:8px">Basisgröße bleibt unverändert. Simulation aus den letzten Capital-TRADE-Transaktionen: PF&lt;0,9 → 0,50× · 0,9–1,2 → 0,75× · 1,2–1,8 → 1,00× · 1,8–2,5 → 1,10× · ≥2,5 → 1,25×. Unter 6 abgeschlossenen Trades immer 1,00×.</div><div class="qcap-table-wrap"><table class="qcap-table"><thead><tr><th>Instrument</th><th>Basis-Size</th><th>Closed</th><th>Rolling PF</th><th>Rolling NET</th><th>Faktor</th><th>Sim Size</th><th>AUTO</th></tr></thead><tbody>${simRows}</tbody></table></div></div>`;
      ...
      const tradesHtml=`<div class="qcap-card"><div class="qcap-title">GESCHLOSSENE TRADES · CAPITAL · LETZTE 7 TAGE</div><div class="qcap-table-wrap"><table class="qcap-table"><thead><tr><th>Zeit</th><th>Instrument</th><th>Notiz</th><th>Betrag</th><th>Währung</th><th>Referenz</th><th>Status</th></tr></thead><tbody>${tradeRows}</tbody></table></div></div>`;
      body.innerHTML=cards+positionsHtml+simHtml+tradesHtml;
