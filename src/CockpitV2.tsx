...
export default function CockpitV2(){
 ...
 useEffect(()=>{const pts=research?.trend_macd_points||[];let j=-1;priceSeries.current?.setData(shown.map(x=>{while(j+1<pts.length&&Number(pts[j+1].time)<=Number(x.time))j++;const cur=j>=0?pts[j]:null;const dir=cur?Number(cur.trend||0):0;const base={time:x.time as Time,open:x.open,high:x.high,low:x.low,close:x.close};if(dir===0)return base;const color=dir>0?"#22c55e":"#ef4444";return {...base,color,borderColor:color,wickColor:color}}));},[shown,research?.trend_macd_points]);
