 ...
 return <div style={{height:"calc(100vh - 84px)",minHeight:720,display:"grid",gridTemplateRows:"auto 1fr",gap:8,padding:8,color:"#dbe4ff",background:"#050914"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 365px",gap:8,minHeight:0}}>
   <div style={{...panel,overflow:"hidden",position:"relative",minHeight:0}}>
    ...
    <div style={{position:"absolute",top:8,left:10,zIndex:3,padding:"5px 8px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:800}}>{symbol} · VIEW {interval} · RESEARCH LIVE · TREND {trendMacdMinutes}m MACD 24/59 · TX {exitCfg.trendExitEnabled?"ON":"OFF"} · E1 {exitCfg.exit1Tf} · E2 {exitCfg.exit2Tf} · E3 {exitCfg.exit3Tf} · E4 {exitCfg.exit4Tf}</div><div style={{position:"absolute",top:42,right:10,zIndex:4,display:"flex",gap:5,padding:"5px 7px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:900}}>{([120,180,240] as const).map(m=><button key={m} onClick={()=>{setTrendMacdMinutes(m);queueMicrotask(()=>void runResearch(profileRef.current,intervalRef.current,true));}} style={{...buttonStyle,padding:"3px 8px",background:trendMacdMinutes===m?"#14532d":"#0a1020",color:trendMacdMinutes===m?"#86efac":"#cbd5e1"}}>{m}m</button>)}</div><div style={{position:"absolute",top:42,left:10,zIndex:3,display:"flex",gap:5,padding:"5px 7px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:900}}>{([["5m","5m"],["15m","15m"],["30m","30m"],["1h","60m"],["2h","120m"]] as const).map(([key,label])=>{const x=research?.rsi_tf_directions?.[key];const d=Number(x?.direction||0);return <span key={key} title={x?("RSI "+Number(x.value).toFixed(2)):"noch kein Wert"} style={{padding:"2px 5px",borderRadius:4,border:"1px solid #334155",color:d>0?"#22c55e":d<0?"#ef4444":"#94a3b8"}}>{label} {d>0?"↑":d<0?"↓":"·"}</span>})}</div>
   </div>
