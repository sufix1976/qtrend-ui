...
export default function CockpitV2(){
 ...
 useEffect(()=>{const pts=research?.trend_rsi_points||[];let j=0;priceSeries.current?.setData(shown.map(x=>{while(j+1<pts.length&&Number(pts[j+1].time)<=Number(x.time))j++;const a=pts[j],b=pts[Math.min(j+1,pts.length-1)],dir=a&&b&&Number(b.value)!==Number(a.value)?(Number(b.value)>Number(a.value)?1:-1):(Number(a?.trend)||1);const color=dir>0?"#22c55e":"#ef4444";return {time:x.time as Time,open:x.open,high:x.high,low:x.low,close:x.close,color,borderColor:color,wickColor:color}}));},[shown,research?.trend_rsi_points]);
 ...
 return <div style={{height:"calc(100vh - 84px)",minHeight:720,display:"grid",gridTemplateRows:"auto 1fr",gap:8,padding:8,color:"#dbe4ff",background:"#050914"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 365px",gap:8,minHeight:0}}>
   <div style={{...panel,overflow:"hidden",position:"relative",minHeight:0}}>
    ...
    <div style={{position:"absolute",top:8,left:10,zIndex:3,padding:"5px 8px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:800}}>{symbol} · VIEW {interval} · RESEARCH LIVE · TREND 2H RSI(14) · ZONE {channelCfg.showTrendZone?`ON ${Number(channelCfg.trendZoneSensitivity).toFixed(2)}`:"OFF"} · TX {exitCfg.trendExitEnabled?"ON":"OFF"} · E1 {exitCfg.exit1Tf} · E2 {exitCfg.exit2Tf} · E3 {exitCfg.exit3Tf} · E4 {exitCfg.exit4Tf}</div>
   </div>
