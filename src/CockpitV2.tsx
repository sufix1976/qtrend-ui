...
type ResearchResult={version:number;mode:"RESEARCH";symbol:string;generated_at:number;duration_ms:number;view_tf:string;base_candle_count:number;profile:any;chart_candles:Candle[];entry_rows:any[];exit_rows:any[];candidate_exit_rows:any[];channel_rows:any[];controller_rows:any[];channel_line:any[];trend_rsi_points?:Array<{time:number;value:number;trend:number}>;rsi_tf_directions?:Record<string,{time:number;value:number;direction:number}|null>;squeeze_points:SqueezePoint[];lrc_points:Point[];fisher_points:Point[];rsi_exit_points:Point[];backtest:BacktestStats&{closedTrades?:any[]};latest_controller:any};
...
export default function CockpitV2(){
 ...
 useEffect(()=>{const pts=research?.trend_rsi_points||[];let j=-1;priceSeries.current?.setData(shown.map(x=>{while(j+1<pts.length&&Number(pts[j+1].time)<=Number(x.time))j++;const next=j>=0&&j+1<pts.length?pts[j+1]:null,cur=j>=0?pts[j]:null;const dir=cur&&next?(Number(next.value)>Number(cur.value)?1:Number(next.value)<Number(cur.value)?-1:0):0;const base={time:x.time as Time,open:x.open,high:x.high,low:x.low,close:x.close};if(dir===0)return base;const color=dir>0?"#22c55e":"#ef4444";return {...base,color,borderColor:color,wickColor:color}}));},[shown,research?.trend_rsi_points]);
 useEffect(()=>{liveLrcSeries.current?.applyOptions({visible:false});liveLrcSeries.current?.setData([]);},[]);
 useEffect(()=>{trendSeries.current?.applyOptions({visible:false});trendSeries.current?.setData([]);trendRsiSeries.current?.setData((research?.trend_rsi_points||[]).map((p,i,pts)=>{const next=i+1<pts.length?pts[i+1]:null;const slope=next?(Number(next.value)>Number(p.value)?1:Number(next.value)<Number(p.value)?-1:0):0;return {time:Number(p.time) as Time,value:Number(p.value),...(slope===0?{}:{color:slope>0?"#22c55e":"#ef4444"})}}) as any);const pane=chart.current?.panes()?.[5];if(pane)pane.setHeight(150);},[research?.trend_rsi_points]);
