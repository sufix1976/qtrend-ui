...
const LIVE_LRC_LOCKS:Record<string,{id:string;mins:number;n:number}>={DE40:{id:"T37",mins:840,n:7},US30:{id:"T40",mins:720,n:6},US100:{id:"T44",mins:1440,n:4},UK100:{id:"T42",mins:600,n:6},J225:{id:"T43",mins:480,n:6},BTCUSD:{id:"T41",mins:600,n:5},ETHUSD:{id:"T43",mins:480,n:6},GOLD:{id:"T44",mins:1440,n:4},SILVER:{id:"T43",mins:480,n:6},OIL_CRUDE:{id:"T43",mins:480,n:6},US500:{id:"T41",mins:600,n:5},CORN:{id:"T41",mins:600,n:5}};
const liveLrcKey=(symbol:string)=>`qtrend:cockpit-v2:live-lrc:${symbol}`;
function linregLastLive(a:number[],n:number,i:number){if(i<n-1)return a[i]??0;let sx=0,sy=0,sxx=0,sxy=0;for(let j=0;j<n;j++){const y=a[i-n+1+j];sx+=j;sy+=y;sxx+=j*j;sxy+=j*y}const d=n*sxx-sx*sx,m=d?(n*sxy-sx*sy)/d:0,b=(sy-m*sx)/n;return b+m*(n-1)}
function liveLrcTrend(c:Candle[],mins:number,n:number){const sec=mins*60,out:{time:Time;value:number;color:string}[]=[];let closed:Candle[]=[],cur:Candle|null=null,b=-1,dir=1;for(const x of c){const k=Math.floor(Number(x.time)/sec)*sec;if(k!==b){if(cur)closed.push(cur);b=k;cur={time:k,open:x.open,high:x.high,low:x.low,close:x.close,volume:x.volume}}else if(cur){cur.high=Math.max(cur.high,x.high);cur.low=Math.min(cur.low,x.low);cur.close=x.close}if(cur){const w=[...closed,cur],i=w.length-1;if(i>=n-1){const lo=linregLastLive(w.map(z=>z.open),n,i),lc=linregLastLive(w.map(z=>z.close),n,i);dir=lc>=lo?1:-1}}out.push({time:x.time as Time,value:x.close,color:dir>0?"#22c55e":"#ef4444"})}return out}
...
function calculateTrendZones(candles:Candle[],rawLine:any[],cfg:TrendChannelConfig,viewTf:string):TrendZone[]{
 if(!cfg.enabled||!cfg.showTrendZone||candles.length<8||!rawLine?.length)return[];
 const period=Math.max(2,Number(cfg.atrPeriod)||10),sensitivity=Math.max(.05,Number(cfg.trendZoneSensitivity)||.45),sec=tfSeconds(viewTf);
 ...
 let seed=0,prevAtr:number|null=null;
 for(let i=0;i<tr.length;i++){if(prevAtr==null){seed+=tr[i];if(i===period-1){prevAtr=seed/period;atr[i]=prevAtr;}}else{prevAtr=(prevAtr*(period-1)+tr[i])/period;atr[i]=prevAtr;}}
 const line=[...rawLine].map((p:any)=>({time:Number(p.time),value:Number(p.value),trend:(Number(p.trend)===-1?-1:1) as 1|-1})).filter((p:any)=>Number.isFinite(p.time)&&Number.isFinite(p.value)).sort((a:any,b:any)=>a.time-b.time);
 const lineValues:Array<number|null>=Array(candles.length).fill(null);
 const trends:Array<1|-1|null>=Array(candles.length).fill(null);
 const dist:Array<number|null>=Array(candles.length).fill(null);
 const slope:Array<number|null>=Array(candles.length).fill(null);
 let j=-1;
 for(let i=0;i<candles.length;i++){const knownAt=candles[i].time+sec;while(j+1<line.length&&line[j+1].time<=knownAt)j++;if(j<0||atr[i]==null||Number(atr[i])<=0)continue;lineValues[i]=line[j].value;trends[i]=line[j].trend;dist[i]=Math.abs(candles[i].close-line[j].value)/Number(atr[i]);if(i>=3&&lineValues[i-3]!=null&&trends[i-3]===trends[i])slope[i]=Math.abs(Number(lineValues[i])-Number(lineValues[i-3]))/Number(atr[i])/3;}
 const zones:TrendZone[]=[];
 for(let i=6;i<candles.length;i++){const d=dist[i],s=slope[i],oldD=dist[i-3],oldS=slope[i-3],trend=trends[i];if(d==null||s==null||oldD==null||oldS==null||trend==null||trends[i-3]!==trend)continue;const closeToLine=d<=sensitivity,flatLine=s<=Math.max(.02,sensitivity*.25),losing=d<=oldD*.94||s<=oldS*.85;if(!(closeToLine&&flatLine&&losing))continue;const from=candles[i].time,to=from+sec,last=zones.at(-1);if(last&&last.trend===trend&&from<=last.to+1)last.to=to;else zones.push({from,to,trend});}
 return zones;
}
...
export default function CockpitV2(){
 ...
 const [liveLrcEnabled,setLiveLrcEnabled]=useState(()=>{try{return localStorage.getItem(liveLrcKey(symbol))==="1"}catch{return false}});
 const chartHost=useRef<HTMLDivElement>(null),liveLrcSeries=useRef<ISeriesApi<"Line">|null>(null),zoneLayer=useRef<HTMLDivElement>(null),chart=useRef<IChartApi|null>(null),priceSeries=useRef<ISeriesApi<"Candlestick">|null>(null),trendSeries=useRef<ISeriesApi<"Line">|null>(null),sqzSeries=useRef<ISeriesApi<"Histogram">|null>(null),lrcSeries=useRef<ISeriesApi<"Line">|null>(null),fisherSeries=useRef<ISeriesApi<"Line">|null>(null),rsiSeries=useRef<ISeriesApi<"Line">|null>(null),markerApi=useRef<any>(null),candleMapRef=useRef(new Map<number,Candle>()),researchSeq=useRef(0),researchBusy=useRef(false),researchQueued=useRef(false),queuedProfile=useRef<InstrumentProfile|null>(null),queuedViewTf=useRef<string|null>(null),queuedSupersede=useRef(false),symbolRef=useRef(symbol),viewLastTime=useRef(0),tickBusy=useRef(false),profileRef=useRef(profile),intervalRef=useRef(interval);
 ...
 useEffect(()=>{try{setLiveLrcEnabled(localStorage.getItem(liveLrcKey(symbol))==="1")}catch{setLiveLrcEnabled(false)}void bootstrap();},[symbol]);
 ...
 useEffect(()=>{liveLrcSeries.current?.applyOptions({visible:liveLrcEnabled});liveLrcSeries.current?.setData(liveLrcData as any);},[liveLrcData,liveLrcEnabled]);
 ...
 useEffect(()=>{const c=chart.current,layer=zoneLayer.current;if(!c||!layer)return;let raf=0;const draw=()=>{cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{layer.replaceChildren();if(!channelCfg.showTrendZone)return;const ts=c.timeScale();for(const z of trendZones){const x1=ts.timeToCoordinate(z.from as Time),x2=ts.timeToCoordinate(z.to as Time);if(x1==null&&x2==null)continue;const left=Math.max(0,Math.min(x1??0,x2??layer.clientWidth)),right=Math.min(layer.clientWidth,Math.max(x1??0,x2??layer.clientWidth));if(right-left<1)continue;const el=document.createElement("div");el.style.position="absolute";el.style.left=`${left}px`;el.style.width=`${right-left}px`;el.style.top="0";el.style.bottom="0";el.style.background=z.trend===1?"rgba(250,204,21,.11)":"rgba(251,146,60,.11)";el.style.borderLeft=z.trend===1?"1px solid rgba(250,204,21,.22)":"1px solid rgba(251,146,60,.22)";layer.appendChild(el);}});};draw();c.timeScale().subscribeVisibleLogicalRangeChange(draw);const ro=new ResizeObserver(draw);ro.observe(layer);return()=>{cancelAnimationFrame(raf);c.timeScale().unsubscribeVisibleLogicalRangeChange(draw);ro.disconnect();layer.replaceChildren();};},[trendZones,channelCfg.showTrendZone]);
 ...
 const patchChannel=(p:Partial<TrendChannelConfig>)=>change(v=>({...v,modules:{...v.modules,channel:{...v.modules.channel,...p}}}));
...
function ChannelSettings({cfg,patch,liveLrcEnabled,setLiveLrcEnabled,symbol,liveLrcLock}:{cfg:TrendChannelConfig;patch:(p:Partial<TrendChannelConfig>)=>void;liveLrcEnabled:boolean;setLiveLrcEnabled:(x:boolean)=>void;symbol:string;liveLrcLock?:{id:string;mins:number;n:number}}){return <div style={{display:"grid",gap:9}}><div style={{padding:8,border:"1px solid #166534",borderRadius:7,color:"#86efac",fontWeight:900}}>HART: Trend LONG → nur LONG öffnen · Trend SHORT → nur SHORT öffnen. Exit darf schließen.</div><div style={{padding:9,border:"1px solid #0e7490",borderRadius:8,display:"grid",gap:6,background:"#08334455"}}><b style={{color:"#67e8f9"}}>LIVE-LRC · NEUER TREND</b><label>LIVE-LRC verwenden <input type="checkbox" checked={liveLrcEnabled} onChange={e=>{const on=e.target.checked;setLiveLrcEnabled(on);patch({liveLrcEnabled:on});try{localStorage.setItem(liveLrcKey(symbol),on?"1":"0")}catch{}}}/></label><small style={{color:"#a5f3fc"}}>{liveLrcLock?`${liveLrcLock.id} · ${liveLrcLock.mins/60}h · L${liveLrcLock.n}`:"Kein Lock"} · grün/rot im Chart · derzeit NUR ANZEIGE</small></div><div style={{padding:9,border:"1px solid #475569",borderRadius:8,display:"grid",gap:6}}><b>CHANNEL 1 · ALTER TREND</b><label>TREND TF <select value={cfg.tf} onChange={e=>patch({tf:e.target.value})} style={inputStyle}>{INTERVALS.map(x=><option key={x}>{x}</option>)}</select></label><label>Channel 1 aktiv <input type="checkbox" checked={cfg.enabled} onChange={e=>patch({enabled:e.target.checked})}/></label></div><label>Linie <input type="checkbox" checked={cfg.showLines} onChange={e=>patch({showLines:e.target.checked})}/></label><label>ATR Periode <input type="number" value={cfg.atrPeriod} onChange={e=>patch({atrPeriod:Number(e.target.value)})} style={inputStyle}/></label><label>Multiplikator <input type="number" step="0.1" value={cfg.multiplier} onChange={e=>patch({multiplier:Number(e.target.value)})} style={inputStyle}/></label><label>Wilder ATR <input type="checkbox" checked={cfg.useRmaAtr} onChange={e=>patch({useRmaAtr:e.target.checked})}/></label><div style={{padding:9,border:"1px solid #854d0e",borderRadius:8,display:"grid",gap:7,background:"#42200655"}}><b style={{color:"#fde68a"}}>TRENDZONE · NUR ANZEIGE</b><label>Zone anzeigen <input type="checkbox" checked={cfg.showTrendZone} onChange={e=>patch({showTrendZone:e.target.checked})}/></label><label>Empfindlichkeit (ATR) <input type="number" min="0.05" max="1.50" step="0.05" value={cfg.trendZoneSensitivity} onChange={e=>patch({trendZoneSensitivity:Math.max(.05,Number(e.target.value)||.45)})} style={inputStyle}/></label><small style={{color:"#fcd34d"}}>Kausal: Abstand Preis→Channel + Channel-Steigung, jeweils ATR-normalisiert. Höher = mehr/breitere Zonen. Hat keinerlei Einfluss auf Controller, Entry, Exit oder Orders.</small></div></div>}
