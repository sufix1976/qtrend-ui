...
type ResearchResult={version:number;mode:"RESEARCH";symbol:string;generated_at:number;duration_ms:number;view_tf:string;base_candle_count:number;profile:any;chart_candles:Candle[];entry_rows:any[];exit_rows:any[];candidate_exit_rows:any[];channel_rows:any[];controller_rows:any[];channel_line:any[];trend_macd_points?:Array<{time:number;value:number;trend:number}>;rsi_tf_directions?:Record<string,{time:number;value:number;direction:number}|null>;squeeze_points:SqueezePoint[];lrc_points:Point[];fisher_points:Point[];rsi_exit_points:Point[];backtest:BacktestStats&{closedTrades?:any[]};latest_controller:any};
...
function normalizeProfile(symbol:string,raw:any,ui?:InstrumentProfile):InstrumentProfile{return{...defaultProfile(symbol),...(raw||{}),symbol,chartMode:ui?.chartMode||raw?.chartMode||"candles",activeModule:ui?.activeModule||raw?.activeModule||"exit",modules:{entry:{...DEFAULT_ENTRY,...(raw?.modules?.entry||{})},exit:{...DEFAULT_EXIT,...(raw?.modules?.exit||{})},channel:{...DEFAULT_CHANNEL,...DEFAULT_TREND_ZONE,...(raw?.modules?.channel||{})},poc:{...(raw?.modules?.poc||{})},controller:{...DEFAULT_CONTROLLER,...(raw?.modules?.controller||{})}}};}
...
export default function CockpitV2(){
 ...
 async function refreshExecutionMarks(){try{const currentSymbol=symbolRef.current,j=await fetchJson(`${BACKEND_BASE}/ui/strategy-events?symbol=${encodeURIComponent(currentSymbol)}&newest=1&_ts=${Date.now()}`);if(currentSymbol!==symbolRef.current)return;const rows=(Array.isArray(j?.rows)?j.rows:[]).filter((r:any)=>String(r?.source||"")==="cockpit_v2_execution_status");const marks:ExecutionMark[]=[];for(const row of rows){let x:any=null;try{x=JSON.parse(String(row?.reason||"{}"));}catch{continue;}const label=String(x?.label||"").trim();if(!label)continue;marks.push({id:Number(row?.id||0),time:Number(row?.time||0),label,status:String(x?.status||""),action:String(x?.action||""),eventId:String(x?.event_id||"")});}setExecutionMarks(marks.sort((a,b)=>a.time-b.time||a.id-b.id).slice(-300));}catch{}}
 ...
 async function runResearch(p:InstrumentProfile=profileRef.current,viewTf:string=intervalRef.current,supersede=false){
  ...
  setResearching(true);
  try{
   const j=await fetchJson(`${BACKEND_BASE}/cockpit-v2/research`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({symbol:requestSymbol,view_tf:viewTf,trend_macd_minutes:trendMacdMinutes,profile:{...p,symbol:requestSymbol}})});
 ...
 const patchController=(p:Partial<ControllerConfig>)=>change(v=>({...v,modules:{...v.modules,controller:{...v.modules.controller,...p}}}));
 ...
 return <div style={{height:"calc(100vh - 84px)",minHeight:720,display:"grid",gridTemplateRows:"auto 1fr",gap:8,padding:8,color:"#dbe4ff",background:"#050914"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 365px",gap:8,minHeight:0}}>
   <div style={{...panel,overflow:"hidden",position:"relative",minHeight:0}}>
    ...
    <div style={{position:"absolute",top:8,left:10,zIndex:3,padding:"5px 8px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:800}}>{symbol} · VIEW {interval} · RESEARCH LIVE · TREND {controllerCfg.requireTrend?`${trendMacdMinutes}m MACD 24/59`:"AUS"} · TX {exitCfg.trendExitEnabled?"ON":"OFF"} · E1 {exitCfg.exit1Tf} · E2 {exitCfg.exit2Tf} · E3 {exitCfg.exit3Tf} · E4 {exitCfg.exit4Tf}</div><div style={{position:"absolute",top:42,right:10,zIndex:4,display:"flex",gap:5,padding:"5px 7px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:900}}><button onClick={()=>patchController({requireTrend:false})} style={{...buttonStyle,padding:"3px 8px",background:!controllerCfg.requireTrend?"#7f1d1d":"#0a1020",color:!controllerCfg.requireTrend?"#fecaca":"#cbd5e1"}}>AUS</button>{([120,180,240] as const).map(m=><button key={m} onClick={()=>{patchController({requireTrend:true});setTrendMacdMinutes(m);queueMicrotask(()=>void runResearch(profileRef.current,intervalRef.current,true));}} style={{...buttonStyle,padding:"3px 8px",background:controllerCfg.requireTrend&&trendMacdMinutes===m?"#14532d":"#0a1020",color:controllerCfg.requireTrend&&trendMacdMinutes===m?"#86efac":"#cbd5e1"}}>{m}m</button>)}</div><div style={{position:"absolute",top:42,left:10,zIndex:3,display:"flex",gap:5,padding:"5px 7px",borderRadius:6,background:"#08111ecc",border:"1px solid #23324a",fontSize:11,fontWeight:900}}>{([["5m","5m"],["15m","15m"],["30m","30m"],["1h","60m"],["2h","120m"]] as const).map(([key,label])=>{const x=research?.rsi_tf_directions?.[key];const d=Number(x?.direction||0);return <span key={key} title={x?("RSI "+Number(x.value).toFixed(2)):"noch kein Wert"} style={{padding:"2px 5px",borderRadius:4,border:"1px solid #334155",color:d>0?"#22c55e":d<0?"#ef4444":"#94a3b8"}}>{label} {d>0?"↑":d<0?"↓":"·"}</span>})}</div>
   </div>
