...
export default function CockpitV2(){
 ...
 useEffect(()=>{trendSeries.current?.applyOptions({visible:false});trendSeries.current?.setData([]);trendRsiSeries.current?.setData((research?.trend_rsi_points||[]).map((p,i,pts)=>{const prev=i>0?pts[i-1]:null;const slope=prev?(Number(p.value)>Number(prev.value)?1:Number(p.value)<Number(prev.value)?-1:0):0;return {time:Number(p.time) as Time,value:Number(p.value),...(slope===0?{}:{color:slope>0?"#22c55e":"#ef4444"})}}) as any);const pane=chart.current?.panes()?.[5];if(pane)pane.setHeight(150);},[research?.trend_rsi_points]);
