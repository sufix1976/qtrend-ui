...
type MlStatus={annotations?:{total:number;good:number;bad:number};trained_rows?:number;new_examples?:number;model_current?:boolean;ml?:{training?:{trained_at?:string;walk_forward_summary?:{auc?:number;precision_good?:number;recall_good?:number;balanced_accuracy?:number}};model_exists?:boolean};ml_error?:string};
...
export default function Trainer(){
 ...
 async function loadMlStatus(){try{const r=await fetch(`${BACKEND_BASE}/trainer/ml/status`,{cache:"no-store"});const j=await r.json();if(r.ok&&j.ok)setMlStatus(j);else setTrainingMessage(j.error||"KI-Status nicht verfügbar");}catch(e:any){setTrainingMessage(e.message||String(e));}}
 async function retrainMl(){if(training)return;setTraining(true);setTrainingMessage("Datensatz wird synchronisiert, Walk-Forward und Training laufen …");try{const r=await fetch(`${BACKEND_BASE}/trainer/ml/retrain`,{method:"POST"});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||`HTTP ${r.status}`);const wf=j.walk_forward||{};setTrainingMessage(`Training fertig · AUC ${Number(wf.auc||0).toFixed(3)} · GOOD-Präzision ${(Number(wf.precision_good||0)*100).toFixed(1)} %`);await loadMlStatus();}catch(e:any){setTrainingMessage(`Training fehlgeschlagen: ${e.message||String(e)}`);}finally{setTraining(false)}}
 ...
 return <div className="trainer-shell">
  ...
  <div className="rating"><span>Stärke</span>{[1,2,3,4,5].map(n=><button className={rating>=n?"on":""} key={n} onClick={()=>setRating(n)}>★</button>)}</div><textarea placeholder="Notiz optional, z. B. sauberer Knick / zu spät / Seitwärts" value={note} onChange={e=>setNote(e.target.value)}/><div className="judge"><button className="good" onClick={()=>save("good")}>G · GUT</button><button className="bad" onClick={()=>save("bad")}>S · SCHLECHT</button><button className="unsure" onClick={()=>save("unsure")}>U · UNSICHER</button>{mode==="manual"&&<button onClick={()=>save("no_trade")}>NO TRADE</button>}</div><div className="ml-training-card"><div className="ml-training-head"><div><strong>Trainer-KI</strong><span>{mlStatus?.model_current?"Modell aktuell":"Modell veraltet"}</span></div><span className={mlStatus?.model_current?"ml-dot current":"ml-dot stale"}/></div><div className="ml-training-grid"><span>Datensatz</span><b>{mlStatus?.annotations?.total??"–"}</b><span>GOOD / BAD</span><b>{mlStatus?.annotations?.good??"–"} / {mlStatus?.annotations?.bad??"–"}</b><span>Trainiert</span><b>{mlStatus?.trained_rows??0}</b><span>Neu</span><b>{mlStatus?.new_examples??0}</b></div><button className="ml-train-button" disabled={training||!mlStatus?.annotations?.total} onClick={retrainMl}>{training?"KI WIRD TRAINIERT …":"KI NEU TRAINIEREN"}</button>{trainingMessage&&<div className="ml-training-message">{trainingMessage}</div>}{mlStatus?.ml_error&&<div className="trainer-error">ML-Dienst: {mlStatus.ml_error}</div>}</div>{error&&<div className="trainer-error">{error}</div>}<small>Tastatur: G / S / U · Pfeile für Kandidaten</small></aside></main>
 </div>
}
