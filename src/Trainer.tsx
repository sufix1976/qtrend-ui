...
export default function Trainer(){
 ...
 async function save(label:"good"|"bad"|"unsure"|"no_trade"){
  ...
  const r=await fetch(`${BACKEND_BASE}/trainer/annotation`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({symbol,interval,...point,label,rating,note})});const j=await r.json();if(!r.ok||!j.ok){setError(j.error||"Speichern fehlgeschlagen");return}setAnnotations(prev=>[...prev.filter(a=>!(a.time===j.annotation.time&&a.side===j.annotation.side&&a.source===j.annotation.source)),j.annotation]);setNote("");if(mode==="auto"&&cursor<candidates.length-1)setCursor(x=>x+1);
 }
 ...
 return <div className="trainer-shell">
  ...
  <div className="rating"><span>Stärke</span>{[1,2,3,4,5].map(n=><button className={rating>=n?"on":""} key={n} onClick={()=>setRating(n)}>★</button>)}</div><textarea placeholder="Notiz optional, z. B. sauberer Knick / zu spät / Seitwärts" value={note} onChange={e=>setNote(e.target.value)}/><div className="judge"><button className="good" onClick={()=>save("good")}>G · GUT</button><button className="bad" onClick={()=>save("bad")}>S · SCHLECHT</button><button className="unsure" onClick={()=>save("unsure")}>U · UNSICHER</button>{mode==="manual"&&<button onClick={()=>save("no_trade")}>NO TRADE</button>}</div>{error&&<div className="trainer-error">{error}</div>}<small>Tastatur: G / S / U · Pfeile für Kandidaten</small></aside></main>
 </div>
}
