...
export default function ExtremeLiveCockpit({ chartOnly = false }: { chartOnly?: boolean }){
  ...
  async function runChainTest(action:"LONG"|"SHORT"|"EXIT"){
    ...
    if(!auto){
      const msg=`AUTO ist für ${symbol} ${interval} laut geladener Konfiguration AUS`;
      setChainMessage(msg);setStatus(msg);return;
    }
    try{
      setChainBusy(true);
      ...
      if(!eventId) throw new Error(`Engine antwortete ohne event_id: ${JSON.stringify(created)}`);

      setChainTest({...created,status:"event_created"});
      setChainMessage(`ENGINE OK · ${action}-Event erstellt · ${eventId.slice(-18)}`);
      ...
      while(Date.now()<deadline){
        ...
        if(test?.status==="event_created"){
          setChainMessage(`ENGINE OK · warte auf Worker · ${eventId.slice(-18)}`);
        }else if(test?.status==="worker_received"){
          setChainMessage(`WORKER OK · Event gelesen · warte auf Weiterleitung`);
        }else if(test?.status==="strategy_forwarded"){
          ...
          const msg=`FEHLER: ${test?.error||"unbekannt"}`;
          setChainMessage(msg);
      ...
      setChainMessage(`TIMEOUT · Event existiert, aber Kette nach 60 Sekunden nicht fertig`);
      setStatus(`Kettentest ${action}: Statusprüfung nach 60 Sekunden beendet`);
    }catch(e){
      const msg=e instanceof Error?e.message:String(e);
      console.error("[WORKER CHAIN TEST] failed",e);
      setChainMessage(`ENGINE FEHLER · ${msg}`);
      setStatus(`Kettentest fehlgeschlagen: ${msg}`);
    }finally{
      setChainBusy(false);
    }
  }
  ...
  return <div style={{minHeight:"100vh",background:"#050914",color:"#eef2ff",fontFamily:"Inter,Arial,sans-serif",padding:10,boxSizing:"border-box"}}>
    ...
    <main style={{display:"grid",gridTemplateColumns:chartOnly?"minmax(0,1fr)":"minmax(0,1fr) 330px",gap:10,minHeight:"calc(100vh - 80px)"}}>
      ...
      {!chartOnly && <aside style={{display:"grid",gap:9,alignContent:"start",maxHeight:"calc(100vh - 90px)",overflowY:"auto",paddingRight:3}}>
        ...
        <Card title="WORKER KETTENTEST"><div style={{fontSize:11,color:"#94a3b8",marginBottom:8}}>Nur GOLD · DEMO · läuft über Engine-Event → Pure Event Worker → Broker.</div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}><button type="button" disabled={chainBusy||symbol!=="GOLD"||!auto} style={{...longButton,opacity:chainBusy?0.55:1}} onClick={()=>{void runChainTest("LONG");}}>TEST LONG</button><button type="button" disabled={chainBusy||symbol!=="GOLD"||!auto} style={{...shortButton,opacity:chainBusy?0.55:1}} onClick={()=>{void runChainTest("SHORT");}}>TEST SHORT</button><button type="button" disabled={chainBusy||symbol!=="GOLD"||!auto} style={{...flatButton,opacity:chainBusy?0.55:1}} onClick={()=>{void runChainTest("EXIT");}}>TEST EXIT</button></div><div style={{fontSize:11,fontWeight:800,color:chainMessage.startsWith("ENGINE FEHLER")||chainMessage.startsWith("FEHLER")||chainMessage.startsWith("TIMEOUT")?"#f87171":chainMessage.startsWith("KETTE OK")?"#22c55e":"#facc15",marginTop:9,padding:"7px 8px",background:"#08101d",border:"1px solid #26344d",borderRadius:7,overflowWrap:"anywhere"}}>{chainBusy?"● ":""}{chainMessage}</div>{chainTest?<div style={{marginTop:9}}><Row k="Engine Event" v={chainStage("event_created")?"OK":"WAIT"}/><Row k="Worker gelesen" v={chainStage("worker_received")?"OK":"WAIT"}/><Row k="Strategy State" v={chainStage("strategy_forwarded")?"OK":"WAIT"}/><Row k="Broker" v={chainStage("broker_confirmed")?"OK":chainTest.status==="failed"?"FEHLER":"WAIT"}/><Row k="Aktion" v={String(chainTest.action||chainTest.target_state||"–").toUpperCase()}/><Row k="Test-ID" v={String(chainTest.event_id||"–").slice(-18)}/>{chainTest.error?<div style={{fontSize:11,color:"#f87171",marginTop:6}}>{String(chainTest.error)}</div>:null}</div>:<div style={{fontSize:11,color:"#64748b",marginTop:8}}>{symbol!=="GOLD"?"Bitte GOLD auswählen.":!auto?"AUTO muss eingeschaltet sein.":null}</div>}</Card>
