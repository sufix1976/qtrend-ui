...
export default function ProfileLab(){
  ...
  useEffect(()=>{
    ...
    markerApi.current?.setMarkers(events);
  ...
  async function loadProfiles(){
    try{
      ...
      const next={...DEFAULT_PARAMS,...(data.active_params||list[0]?.params||{})};
      setParams(next);
      setBaseline(next);
      setDirty(false);
      setSelectedId(list[0]?.id||"");
      setProfileName(`${symbol} ${interval} · Manuell`);
      setStatus(data.active_params?"Aktives Profil geladen":"Kein aktives Profil – Standardwerte geladen");
    }catch(error){
  ...
  async function runPreview(showStatus=true){
    try{
      ...
      if(showStatus)setStatus("Vorschau aktualisiert · nicht gespeichert · kein Handel");
    }catch(error){
  ...
  async function activateSelected(){
    ...
    if(!profile){setStatus("Bitte ein gespeichertes Profil auswählen");return;}
    try{
      setBusy(true);
  ...
  return <div style={page}>
    <div style={topbar}>
      ...
      <div style={{
        ...
        {dirty?"TEMPORÄR · NICHT GESPEICHERT":"ARBEITSSTAND GESPEICHERT"}
      </div>
    </div>
      ...
      <aside style={side}>
        <Card title="AUSGANGSPUNKT">
          ...
          <div style={buttonGrid}>
            <button style={secondaryButton} onClick={()=>chosen&&loadProfile(chosen)}>PROFIL LADEN</button>
            <button style={secondaryButton} onClick={loadActive}>AKTIVES LADEN</button>
          </div>
          <button style={primaryButton} disabled={busy} onClick={()=>void runPreview(true)}>
        ...
        <Card title="STRATEGIE">
          <select
            ...
            onChange={e=>{
              const mode=e.target.value as Params["strategy_mode"];
              setParams(prev=>({
                ...prev,
                strategy_mode:mode,
                trend_filter_mode:mode==="basis_ad"?"ad":mode==="basis_chaikin"?"chaikin":"none",
              }));
            }}
          >
        ...
        <Card title="FLOW · AD + CHAIKIN">
          ...
          <select
            ...
            onChange={e=>setField("trend_filter_mode",e.target.value as Params["trend_filter_mode"])}
          >
        ...
        <Card title="MANUELLES PROFIL">
          <input style={inputStyle} value={profileName} onChange={e=>setProfileName(e.target.value)} placeholder="Profilname"/>
          <textarea style={{...inputStyle,minHeight:76,marginTop:7,resize:"vertical"}} value={note} onChange={e=>setNote(e.target.value)} placeholder="Notiz zum Chartbild"/>
          <div style={buttonGrid}>
            <button style={secondaryButton} disabled={busy} onClick={()=>void saveManual(false)}>NUR SPEICHERN</button>
            <button style={activateButton} disabled={busy} onClick={()=>void saveManual(true)}>SPEICHERN + AKTIVIEREN</button>
          </div>
          <button style={activateButton} disabled={busy||!selectedId} onClick={()=>void activateSelected()}>
...
function Metric({label,value}:{label:string;value:string}){
  return <div style={metric}><div style={{fontSize:10,color:"#64748b"}}>{label}</div><b>{value}</b></div>;
}

function Rating({label,value,onChange}:{label:string;value:number;onChange:(value:number)=>void}){
  return <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,padding:"4px 0"}}>
    ...
    <div>
      {[1,2,3,4,5].map(star=><button
        ...
        style={{border:0,background:"transparent",color:star<=value?"#facc15":"#475569",fontSize:17,cursor:"pointer",padding:1}}
      >★</button>)}
    </div>
  </div>;
}

const page:CSSProperties={minHeight:"100vh",background:"#050914",color:"#eef2ff",padding:10,fontFamily:"Inter,Arial,sans-serif",boxSizing:"border-box"};
const topbar:CSSProperties={display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:10};
const badge:CSSProperties={border:"1px solid",borderRadius:999,padding:"8px 12px",fontSize:11,fontWeight:900};
const mainGrid:CSSProperties={display:"grid",gridTemplateColumns:"minmax(0,1fr) 365px",gap:10,alignItems:"start"};
const chartCard:CSSProperties={background:"#0b1220",border:"1px solid #26344d",borderRadius:12,padding:8,minWidth:0};
const side:CSSProperties={display:"grid",gap:9};
const card:CSSProperties={background:"#0b1220",border:"1px solid #26344d",borderRadius:10,padding:10};
const cardTitle:CSSProperties={fontSize:11,fontWeight:900,color:"#93c5fd",marginBottom:8,letterSpacing:.4};
const twoCols:CSSProperties={display:"grid",gridTemplateColumns:"1fr 1fr",gap:7};
const fieldStyle:CSSProperties={display:"grid",gap:3,fontSize:10,color:"#94a3b8"};
const inputStyle:CSSProperties={width:"100%",boxSizing:"border-box",background:"#08101d",border:"1px solid #334155",borderRadius:7,color:"#f8fafc",padding:"7px 8px"};
const selectStyle:CSSProperties={...inputStyle};
const buttonGrid:CSSProperties={display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:7};
const primaryButton:CSSProperties={width:"100%",marginTop:7,padding:"9px 10px",borderRadius:8,border:"1px solid #38bdf8",background:"#075985",color:"#e0f2fe",fontWeight:900,cursor:"pointer"};
const secondaryButton:CSSProperties={padding:"8px 7px",borderRadius:8,border:"1px solid #475569",background:"#172033",color:"#e2e8f0",fontWeight:800,cursor:"pointer"};
const activateButton:CSSProperties={width:"100%",marginTop:7,padding:"9px 8px",borderRadius:8,border:"1px solid #22c55e",background:"#14532d",color:"#dcfce7",fontWeight:900,cursor:"pointer"};
const metricsGrid:CSSProperties={display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6,marginTop:8};
const metric:CSSProperties={background:"#08101d",border:"1px solid #26344d",borderRadius:8,padding:8,textAlign:"center"};
const statusBar:CSSProperties={marginTop:10,background:"#08101d",border:"1px solid #26344d",borderRadius:8,padding:"9px 10px",fontSize:12,color:"#cbd5e1"};
