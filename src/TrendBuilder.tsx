...
const TREND_LOCKS:Record<string,string>={J225:"T32",DE40:"T32",US30:"T30",US100:"T31",UK100:"T27",BTCUSD:"T25",ETHUSD:"T25",GOLD:"T29",SILVER:"T29",OIL_CRUDE:"T31",US500:"T29",CORN:"T32"};
...
function lrcControl(c:C[],mins:number,n:number){const r=resample(c,mins),op=r.map(z=>z.open),cl=r.map(z=>z.close);let dir=1;const t=r.map((_,i)=>{if(i<n-1)return dir;const lo=linregLast(op,n,i),lc=linregLast(cl,n,i);dir=lc>=lo?1:-1;return dir});return expand(c,r,t)}
...
function build(c:C[]):V[]{const out:V[]=[];const defs=[
 ...
 for(const [family,tf,speed,mins,n] of lrcDefs){const full=lrcControl(c,mins,n);out.push({id:"T"+String(out.length+1).padStart(2,"0"),family,speed,trend:full,changes:flips(full),params:`${tf} intern · Linear Regression Length ${n} · KAUSAL`})}
 return out}
export default function TrendBuilder(){
 const [symbol,setSymbol]=useState("J225"),[candles,setCandles]=useState<C[]>([]),[sel,setSel]=useState(31),[busy,setBusy]=useState(false),[err,setErr]=useState("");
 ...
 return <div style={{padding:14,color:"#dbeafe",background:"#050b12",minHeight:"100vh",fontFamily:"Inter,system-ui"}}>
  ...
  <div style={{marginTop:10,color:"#94a3b8",fontSize:12}}>V3.3: Instrumentbezogene Trend-Locks sind fest hinterlegt. Beim Instrumentwechsel wird automatisch dessen gespeicherte Variante geöffnet. Research only – keine Änderung an Live, Controller oder Profilen.</div>
 </div>
}