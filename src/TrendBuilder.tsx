...
function build(c:C[]):V[]{const x=c.map(z=>z.close),out:V[]=[];const speeds=[["LANGSAM",80],["MITTEL",40],["SCHNELL",20]] as const;
 for(const [s,n] of speeds){const sl=regression(x,n),t=carry(sl.map(v=>v>=0?1:-1));out.push({id:"T"+String(out.length+1).padStart(2,"0"),family:"REGRESSION",speed:s,trend:t,changes:flips(t),params:`Slope ${n}`})}
 for(const [s,n] of speeds){const m=ema(x,n),slow=ema(x,n*2),t=carry(x.map((_,i)=>m[i]>=slow[i]?1:-1));out.push({id:"T"+String(out.length+1).padStart(2,"0"),family:"MA STRUCTURE",speed:s,trend:t,changes:flips(t),params:`EMA ${n}/${n*2}`})}
 for(const [s,n] of speeds){let dir=1;const t=x.map((v,i)=>{if(i>=n){let hi=-Infinity,lo=Infinity;for(let j=i-n;j<i;j++){hi=Math.max(hi,c[j].high);lo=Math.min(lo,c[j].low)}if(v>hi)dir=1;else if(v<lo)dir=-1}return dir});out.push({id:"T"+String(out.length+1).padStart(2,"0"),family:"BREAKOUT",speed:s,trend:t,changes:flips(t),params:`Structure ${n}`})}
 for(const [s,n] of speeds){let dir=1;const t=x.map((v,i)=>{if(i<n)return dir;let path=0;for(let j=i-n+1;j<=i;j++)path+=Math.abs(x[j]-x[j-1]);const net=v-x[i-n],er=path?Math.abs(net)/path:0;if(er>=0.22&&net!==0)dir=net>0?1:-1;return dir});out.push({id:"T"+String(out.length+1).padStart(2,"0"),family:"EFFICIENCY",speed:s,trend:t,changes:flips(t),params:`ER ${n} · 0.22`})}
 return out}
export default function TrendBuilder(){
 ...
 return <div style={{padding:14,color:"#dbeafe",background:"#050b12",minHeight:"100vh",fontFamily:"Inter,system-ui"}}>
  <div style={{display:"flex",gap:10,alignItems:"end",flexWrap:"wrap",marginBottom:12}}>
   <div><div style={{fontSize:11,color:"#7dd3fc",fontWeight:800}}>TREND BUILDER V1 · RESEARCH ONLY</div><h2 style={{margin:"3px 0 0"}}>Trend bauen · visuell auswählen · später locken</h2></div>
   <label style={{marginLeft:"auto"}}><div style={{fontSize:11,color:"#94a3b8"}}>Instrument</div><select value={symbol} onChange={e=>setSymbol(e.target.value)} style={{background:"#0b1624",color:"#fff",border:"1px solid #334155",padding:"8px",borderRadius:6}}>{SYMBOLS.map(s=><option key={s}>{s}</option>)}</select></label>
   <button onClick={load} disabled={busy} style={{padding:"8px 12px",background:"#12304b",color:"#fff",border:"1px solid #2563eb",borderRadius:6,fontWeight:800}}>{busy?"LÄDT …":"NEU LADEN"}</button>
  </div>
  ...
  <div style={{marginTop:10,color:"#94a3b8",fontSize:12}}>V1 zeigt 12 kausale Grundvarianten: Regression, MA Structure, Breakout und Efficiency – jeweils langsam, mittel und schnell. Auswahl und Verfeinerung kommen im nächsten Schritt.</div>
 </div>
}