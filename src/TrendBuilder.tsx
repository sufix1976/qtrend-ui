...
function build(c:C[]):V[]{const out:V[]=[];const defs=[
 ...
 const lrcDefs=[["LRC CONTROL","1D","L8",1440,8],["LRC CONTROL","2D","L6",2880,6],["LRC CONTROL","2D","L8",2880,8],["LRC CONTROL","2D","L10",2880,10],["LRC CONTROL","2D","L12",2880,12],["LRC CONTROL","3D","L8",4320,8]] as const;
...
export default function TrendBuilder(){
 ...
 return <div style={{padding:14,color:"#dbeafe",background:"#050b12",minHeight:"100vh",fontFamily:"Inter,system-ui"}}>
  <div style={{display:"flex",gap:10,alignItems:"end",flexWrap:"wrap",marginBottom:12}}><div><div style={{fontSize:11,color:"#7dd3fc",fontWeight:800}}>TREND BUILDER V3 · RESEARCH ONLY</div><h2 style={{margin:"3px 0 0"}}>Große Trendphasen · 1m Chart · interne 30m–4h Entscheidung</h2></div><label style={{marginLeft:"auto"}}><div style={{fontSize:11,color:"#94a3b8"}}>Instrument</div><select value={symbol} onChange={e=>setSymbol(e.target.value)} style={{background:"#0b1624",color:"#fff",border:"1px solid #334155",padding:"8px",borderRadius:6}}>{SYMBOLS.map(s=><option key={s}>{s}</option>)}</select></label><button onClick={load} disabled={busy} style={{padding:"8px 12px",background:"#12304b",color:"#fff",border:"1px solid #2563eb",borderRadius:6,fontWeight:800}}>{busy?"LÄDT …":"NEU LADEN"}</button></div>
  <div style={{display:"grid",gridTemplateColumns:"repeat(9,minmax(65px,1fr))",gap:5,marginBottom:8}}>{vars.map((x,i)=><button key={x.id} onClick={()=>setSel(i)} style={{padding:"7px 3px",background:i===sel?"#164e63":"#0b1624",color:i===sel?"#67e8f9":"#cbd5e1",border:`1px solid ${i===sel?"#0891b2":"#263548"}`,borderRadius:5,fontWeight:800}}>{x.id}</button>)}</div>
  ...
  <div style={{marginTop:10,color:"#94a3b8",fontSize:12}}>V3: T13–T18 sind LRC-Control-Varianten nach dem SpUn-Prinzip, aber streng kausal ohne TradingView-lookahead. T15 entspricht der gewünschten 2D / Length 8 Grundidee.</div>
 </div>
}