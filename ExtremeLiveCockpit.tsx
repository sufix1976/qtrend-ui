...
export default function ExtremeLiveCockpit({ chartOnly = false }: { chartOnly?: boolean }){
  ...
  const [labHandoff,setLabHandoff]=useState<ProfileLabHandoff|null>(null);
  ...
  useEffect(()=>{
    try{
      const raw=localStorage.getItem(LAB_HANDOFF_KEY);
      if(!raw){setLabHandoff(null);return;}
      const parsed=JSON.parse(raw) as ProfileLabHandoff;
      if(
        parsed?.version===1 &&
  ...
  async function load(){
    try{
      ...
      if(labHandoff){
        const previewData=await fetchJson(`${BACKEND_BASE}/qmomentum/profile-lab/preview`,{
          method:"POST",
          headers:{"Content-Type":"application/json"},
          body:JSON.stringify({
            symbol,
            interval,
            params:labHandoff.params,
            limit:1500,
          }),
        });

        setCandles(Array.isArray(previewData.candles)?previewData.candles:[]);
        setSnapshot(s.state||null);
        setProfile({...DEFAULT_PROFILE,...labHandoff.params});
        setLive(previewData);
        setProfiles(Array.isArray(profileList.profiles)?profileList.profiles:[]);
      }else{
        setCandles(Array.isArray(c.candles)?c.candles:[]);
        setSnapshot(s.state||null);
        setProfile({...DEFAULT_PROFILE,...p.params});
        setLive(l);
        setProfiles(Array.isArray(profileList.profiles)?profileList.profiles:[]);
      }
      ...
      setStatus(labHandoff?"TEMPORÄRE PROFILE-LAB-VORSCHAU · aktives Handelsprofil unverändert":"SIGNAL MIRROR verbunden · Auto-Orders noch gesperrt");
    }catch(e){setStatus(`Fehler: ${e instanceof Error?e.message:String(e)}`);}
  }
  useEffect(()=>{void load();const t=window.setInterval(()=>void load(),30000);return()=>window.clearInterval(t);},[symbol,interval,labHandoff]);
  ...
  function endLabPreview(){
    localStorage.removeItem(LAB_HANDOFF_KEY);
    setLabHandoff(null);
    setStatus("Temporäre Lab-Vorschau beendet");
  ...
  return <div style={{minHeight:"100vh",background:"#050914",color:"#eef2ff",fontFamily:"Inter,Arial,sans-serif",padding:10,boxSizing:"border-box"}}>
    ...
    {labHandoff&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,margin:"0 8px 10px",padding:"9px 11px",border:"1px solid #a855f7",borderRadius:8,background:"#3b0764",color:"#f3e8ff",fontWeight:900,fontSize:12}}>
      <span>TEMPORÄRE LAB-VORSCHAU · KEIN HANDELSPROFIL · {symbol} {interval}</span>
      <button type="button" style={{...button(),borderColor:"#c084fc",background:"#581c87"}} onClick={endLabPreview}>VORSCHAU BEENDEN</button>
    </div>}
