const QMOMENTUM_VERSION = "QMOMENTUM_TREND_FORMULA_LAB_V0_2A_STREAMING";
...
function prepareTrendFormulaEvaluation(candles, trendAnnotations) {
  const closes = new Float64Array(candles.length);
  const times = new Float64Array(candles.length);
  const targetState = new Int8Array(candles.length); // 1=up, -1=down, 0=unbekannt
  const timeToIndex = new Map();

  for (let i = 0; i < candles.length; i += 1) {
    closes[i] = finite(candles[i].close);
    times[i] = Number(candles[i].time);
    timeToIndex.set(times[i], i);
  }

  const starts = [...(trendAnnotations || [])]
    .map((row) => ({
      time: Number(row.time),
      state: row.trend_start === "up" ? 1 : -1,
    }))
    .filter((row) => Number.isFinite(row.time))
    .sort((a, b) => a.time - b.time);
  ...
  for (let i = 0; i < candles.length; i += 1) {
    while (cursor < starts.length && starts[cursor].time <= times[i]) {
      current = starts[cursor].state;
      const startIndex = timeToIndex.get(starts[cursor].time);
      if (Number.isInteger(startIndex)) targetStartIndices.push(startIndex);
      cursor += 1;
    }
    targetState[i] = current;
  }

  const tr = new Float64Array(candles.length);
  for (let i = 0; i < candles.length; i += 1) {
    const high = finite(candles[i].high);
    const low = finite(candles[i].low);
    const prevClose = i > 0 ? closes[i - 1] : closes[i];
    tr[i] = Math.max(high - low, Math.abs(high - prevClose), Math.abs(low - prevClose));
  }

  return { closes, times, targetState, targetStartIndices, tr };
}

function emaTyped(values, length) {
  const out = new Float64Array(values.length);
  if (!values.length) return out;
  const alpha = 2 / (Math.max(1, length) + 1);
  out[0] = finite(values[0]);
  for (let i = 1; i < values.length; i += 1) {
    out[i] = alpha * finite(values[i]) + (1 - alpha) * out[i - 1];
  }
  return out;
}

function evaluatePreparedFormula(prepared, params, basis, atr) {
  const { closes, targetState, targetStartIndices } = prepared;

  let state = 0; // 1=up, -1=down, 0=neutral
  let pending = 0;
  let pendingCount = 0;
  let barsInState = 0;
  ...
  // Maximal eine Zahl pro Kerze, keine Objekte.
  ...
  for (const targetIndex of targetStartIndices) {
    ...
    for (const predictedIndex of predictedSwitchIndices) {
      const delta = Math.abs(predictedIndex - targetIndex);
      if (delta < bestDistance) bestDistance = delta;
    }
    if (Number.isFinite(bestDistance)) {
      delaySum += bestDistance;
      delayCount += 1;
  ...
  const accuracy = comparable ? correct / comparable : 0;
  const avgSwitchDistance = delayCount ? delaySum / delayCount : 999;
  const targetSwitchCount = Math.max(1, targetStartIndices.length - 1);
...
function optimizeTrendFormula(candles, trendAnnotations) {
  const prepared = prepareTrendFormulaEvaluation(candles, trendAnnotations);
  const top = [];
  let testedCount = 0;
  ...
  // EMA und ATR werden nur 4 bzw. 3 Mal berechnet – nicht für jede Formel.
  const basisCache = new Map(
    emaLengths.map((length) => [length, emaTyped(prepared.closes, length)]),
  );
  const atrCache = new Map(
    atrLengths.map((length) => [length, emaTyped(prepared.tr, length)]),
  );

  for (const ema_length of emaLengths) {
    const basis = basisCache.get(ema_length);

    for (const atr_length of atrLengths) {
      const atr = atrCache.get(atr_length);
      ...
      for (const min_state_bars of minStateBarsValues) {
        ...
        keepTop(evaluatePreparedFormula(prepared, params, basis, atr));
        testedCount += 1;
      }
    }
  }

  // Volle Zustandsreihe ausschließlich einmal für die Gewinnerformel.
  if (top.length) {
    top[0] = {
      ...top[0],
      states: formulaStateSeries(candles, top[0].params),
    };
  }
    ...
    if (req.method === "GET" && url.pathname === "/qmomentum/formula-optimize") {
      try {
        ...
        sendJson(res, 200, {
          ...
          optimizer_mode: "STREAMING_TYPED_ARRAYS_TOP_10",
